...
const ChooseIND = () => {
  const options = ["METALS", "RAILWAYS", "WIND POWER"];
  const [selectedOption, setSelectedOption] = useState('CHOOSE INDUSTRY');
    ...
    <div className="relative">
      <button
        ...
        className={`flex items-center px-6 py-2 focus:outline-none w-full transition mt-4 text-box-red font-bold ${
          isButtonClicked && (selectedOption === 'CHOOSE INDUSTRY' || isDropdownOpen)
      ...
      >
        ...
        <svg
          xmlns={down}
          className={`ml-auto transform text-black ${isDropdownOpen ? 'text-white rotate-180' : 'rotate-0'} transition-transform duration-300 ease-in-out`}
          viewBox="0 0 20 20"
          fill="currentColor"
          width="40"
          height="40"
      ...
      {isDropdownOpen && (
        ...
        >
          {options.map((option, index) => (
            <div key={index}>
              <li
                className={`px-6 py-2 h-[45px] cursor-pointer hover:bg-slate-200 ${
              ...
              {index !== options.length - 1 && <hr className="border-gray-800 w-full my-2" />}
            </div>
          ))}
        </ul>
      )}
